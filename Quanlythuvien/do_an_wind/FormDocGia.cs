...
namespace do_an_wind
{
    public partial class FormDocGia : Form
    {
        ...
        private void XatNhan_Click(object sender, EventArgs e)
        {
            ...
            if (this.chucNang == 1)
            {
                if (txtCMND.Text.Trim().Length == 12 || txtCMND.Text.Trim().Length == 9)
                {
                    if (txtSDT.Text.Trim().Length == 10)
                    {
                        ...
                        {// DataTable data = BAL.BAL_docGia.getAll();
                            ...
                            if (rdbNam.Checked == true)
                                ...
                                gioitinh = true;
                            BEL.BEL_DocGia docgia = new BEL.BEL_DocGia(
                                txtMaDocGia.Text.Trim(),
                                txtTenDocGia.Text.Trim(),
                                gioitinh,
                            ...
                            {
                                MessageBox.Show("Thêm độc giả thất bại " + txtTenDocGia.Text, "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Error);
                            }
                        }
                        else
                        {
                            MessageBox.Show("Dữ liệu không được để rỗng ", "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Error);
                        }
                    }
                    else
                    {
                ...
                if (txtCMND.Text.Trim().Length == 12 || txtCMND.Text.Trim().Length == 9)
                {
                    if (txtSDT.Text.Trim().Length == 10)
                    {
                        if (!string.IsNullOrEmpty(txtCMND.Text.Trim())
                            ...
                            && !string.IsNullOrEmpty(txtTenDocGia.Text.Trim())
                            && !(dateTimeNgaySinh.Value >= DateTime.Now))
                        {
                            bool gioitinh = false;
                            if (rdbNam.Checked == true)
        ...
        private void txtTenDocGia_TabIndexChanged(object sender, EventArgs e)
        {
            txtTenDocGia.Text = txtTenDocGia.Text.Trim();
        }
    }
}
